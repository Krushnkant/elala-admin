...
class PostController extends BaseController
{
    public function create_post(Request $request){
        ...
        if(isset($request->deleteMedia) && $request->deleteMedia != ""){
            $deleteMedia = explode(",",$request->deleteMedia);
            PostMedia::whereIn('id',$deleteMedia)->delete();
        }
    ...
    public function like_post_users(Request $request){
        ...
        foreach ($postlikes as $postlike){
            ...
            if(isset(Auth::user()->id)) {
                $temp['is_follow'] = is_follower_random(Auth::user()->id,$postlike->user->id);
            }else{
                $temp['is_follow'] = "";
            }
