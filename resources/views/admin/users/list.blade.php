        ...
        <div class="modal-content">
            <form class="form-valide" action="" id="userform" method="post" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label class="col-form-label" for="designation_id">Select Designation <span class="text-danger">*</span>
                        </label>
                        ...
                        <div id="designation_id-error" class="invalid-feedback animated fadeInDown" style="display: none;"></div>
                    </div>
                </div>
...
<div class="modal fade" id="UserFriendListModal">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="formtitle">Friend List User</h5>
                <button type="button" class="close" data-dismiss="modal"><span>×</span>
                </button>
            </div>
            <div class="modal-body" id="friend-list-users">
                <div id="attr-cover-spin" class="cover-spin"></div>
            </div>
            <div class="modal-footer">
                <table id="all_friends" class="table zero-configuration customNewtable" style="width:100%">
                    <thead>
                        <tr>
                            <th>No</th>
                            <th>Image</th>
...
<script type="text/javascript">
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                if (res.status == 'failed') {
                    ...
                    if (res.errors.designation_id) {
                        $('#designation_id-error').show().text(res.errors.designation_id);
                    } else {
                        $('#designation_id-error').hide();
                    }
                }
        ...
        $('#all_users').DataTable({
            ...
            "columns": [{
                ...
                },
                {
                    data: 'profile_pic',
                    name: 'profile_pic',
                    class: "text-center multirow"
                },
                {
                    data: 'user_info',
                    name: 'user_info',
                    class: "text-left multirow",
                    orderable: false
                },
                {
                    data: 'contact_info',
    ...
    $('body').on('click', '#editUserBtn', function() {
        ...
        $.get("{{ url('admin/users') }}" + '/' + user_id + '/edit', function(data) {
            ...
            $('#designation_id').find('option[value="' + data.designation_id + '"]').attr('selected', 'selected').trigger('change');
        })
    });
    ...
    $('body').on('click', '#friendList', function(e) {
        let id = $(this).attr('data-id')
       
        $('#all_friends').DataTable({
            "destroy": true,
            "processing": true,
            "serverSide": true,
            "ajax": {
                "url": "{{ url('admin/allfriendlist') }}/" + id,
                "dataType": "json",
                "type": "POST",
                "data": {
                    _token: '{{ csrf_token() }}',
